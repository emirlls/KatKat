...
public class FlatMemberAppService : KatKatAppService, IFlatMemberAppService
{
    ...
    /// <summary>Batches the User -> UserName/Name/Surname lookup for a whole list, avoiding N+1 queries.</summary>
    private async Task<List<FlatMemberDto>> MapToDtosAsync(List<FlatMember> flatMembers)
    {
        ...
        return flatMembers.Select(flatMember =>
        {
            ...
            var user = userById.GetValueOrDefault(flatMember.UserId);
            dto.UserName = user?.UserName ?? flatMember.UserId.ToString();
            dto.Name = user?.Name;
